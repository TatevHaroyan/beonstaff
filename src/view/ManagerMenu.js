            ...
            <div className="manager-menu">
                ...
                {(prof === "manager" ? this.state.menuManager : this.state.menuAccountant).map((item, index) => {
                    if (item.id === 5 && prof !== "manager") {
                        return <NavLink key={index} activeClassName="active" to={item.url}> <div className='left-item' onClick={() => { this.setState({ active: true }) }} >
                    ...
                    if ((item.id === 4 && prof === "manager") || (item.id === 1 && prof !== "manager")) {
                        return <NavLink key={index} activeClassName="active" to={item.url}> <div className='left-item' onClick={() => { this.setState({ active: true }) }} >
                            <div className="new-notification new-task-menu"><span>{item.title}</span> {notification.new_task ? <div className="tasks-around">{notification.new_task.count}</div> : null}</div>
                            {window.location.href.includes(item.url) ? <div className="menu-line"></div> : null}
                        </div></NavLink>
                    }
                    return <NavLink key={index} activeClassName="active" to={item.url}> <div className='left-item' onClick={() => { this.props.navbarShow() }} >
...
export default connect(
    (state) => ({ word: state.word, show: state.showReducer, employee: state.loginReducer, showNavbar: state.showNavbar, notification: state.notification }),
    (dispatch) => ({
